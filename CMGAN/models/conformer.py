...
# from performer_pytorch.performer_pytorch import FastAttention, orthogonal_matrix_chunk, gaussian_orthogonal_random_matrix
...
class PerformerAttention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None, context_mask = None):
        ...
        # FastAttention doesn't support mask, so we need to handle it differently
        # If you need masking, you might need to implement a custom solution or 
        # use a different attention mechanism
...
class ConformerBlock(nn.Module):
    ...
    def forward(self, x, mask = None): # Input: [B*F,T,64]
        x = self.ff1(x) + x  
