...
def batch_pesq(clean, noisy):
    ...
    if -1 in pesq_score:
        return None
    pesq_score = (pesq_score - 1.0) / 3.5
    return torch.FloatTensor(pesq_score).to('cuda')

# def batch_pesq(clean, noisy,fallback_value=0.5):
#     def safe_pesq(c, n):
#         score = pesq_loss(c, n)
#         if score == -1:
...
#         delayed(safe_pesq)(c, n) for c, n in zip(clean, noisy)
#     )
    
#     pesq_score = np.array(pesq_score, dtype=np.float32)
#     pesq_score = (pesq_score - 1.0) / 3.5
#     pesq_score = np.clip(pesq_score, 0.0, 1.0)
#     return torch.FloatTensor(pesq_score).to("cuda")
